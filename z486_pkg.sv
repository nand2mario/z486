...
// Stage-2 architectural/cache commands generated by protection transitions.
typedef struct packed {
    logic       set_rpl_redirect;
    logic       copy_stack_dpl;
    logic [1:0] copy_dpl;
    logic       conform_dpl;
    logic [1:0] conform_dpl_value;
    logic       write_rpl;
    logic       active;
} prot_transition_t;

// Split base/index scaling into two operands so each caller can retain its
// own adder placement and pipeline boundary.
function automatic logic [63:0] ea_scale_operands(
    input logic [31:0] base_value,
    input logic [31:0] index_value,
    input logic [1:0]  scale,
    input logic        scale_to_base
);
    logic [31:0] base_term, index_term;
    begin
        if (scale_to_base) begin
            case (scale)
                2'b00: base_term = base_value;
                2'b01: base_term = base_value << 1;
                2'b10: base_term = base_value << 2;
                default: base_term = base_value << 3;
            endcase
            index_term = 32'd0;
        end else begin
            base_term = base_value;
            case (scale)
                2'b00: index_term = index_value;
                2'b01: index_term = index_value << 1;
                2'b10: index_term = index_value << 2;
                default: index_term = index_value << 3;
            endcase
        end
        ea_scale_operands = {base_term, index_term};
    end
endfunction
...
// register, descriptor-cache, or x87 interfaces.
typedef struct packed {
    logic jncond;
    logic count_zero;
    logic count_nonzero;
    logic count_low_not_one;
    logic count_not_one;
    logic count_one;
    logic loopne;
    logic greater;
...
typedef struct packed {
    ...
    logic [31:0] disp;
    logic        is16;
    logic        s2b;       // SIB scale-to-base: no index operand
} ea_dec_t;

function automatic logic condition_true(input logic [3:0] cond,
                                        input logic [31:0] flags);
    case (cond)
        4'h0: condition_true = flags[11];
        4'h1: condition_true = !flags[11];
        4'h2: condition_true = flags[0];
        4'h3: condition_true = !flags[0];
        4'h4: condition_true = flags[6];
        4'h5: condition_true = !flags[6];
        4'h6: condition_true = flags[0] | flags[6];
        4'h7: condition_true = !flags[0] & !flags[6];
        4'h8: condition_true = flags[7];
        4'h9: condition_true = !flags[7];
        4'hA: condition_true = flags[2];
        4'hB: condition_true = !flags[2];
        4'hC: condition_true = flags[7] != flags[11];
        4'hD: condition_true = flags[7] == flags[11];
        4'hE: condition_true = flags[6] | (flags[7] != flags[11]);
        4'hF: condition_true = !flags[6] & (flags[7] == flags[11]);
    endcase
endfunction
